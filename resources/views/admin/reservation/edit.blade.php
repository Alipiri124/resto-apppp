<x-admin-layout>
  ...
  <form action="{{route('admin.reservation.update',$reservation->id)}}" method="POST">
    ...
    <div class="sm:col-span-6">
      ...
      <div class="mt-1">
        <input type="number" min="0" id="guest_number" name="guest_number" value="{{$reservation->guest_number}}"
    ...
    <label for="status" class="block text-sm font-medium text-gray-700">Table</label>
      <div class="mt-1">
        <select id="table_id" name="table_id" class="form-multiselect block w-full mt-1">
         ...
         <option value="{{$table->id}}" @selected($table->id == $reservation->table_ids)>{{$table->name}} ({{$table->guest_number}} Guests)</option> 
         @endforeach
         </select>
      </div>
      @error('table_id')
